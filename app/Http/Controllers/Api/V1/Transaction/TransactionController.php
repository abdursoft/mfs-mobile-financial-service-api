...
class TransactionController extends Controller
{
    ...
    public function send(Request $request)
    {
        ...
        if ($payable > $fromUser->transactionLimit->cash_out_max) {
            return response()->json([
        ...
        // check balance
        if ($fromUser->wallet->balance < $payable) {
            return response()->json([
        ...
        try {
            ...
            $this->smsInit("You have sent Tk{$sendAmount} to {$userMask} on {$date} TxID:{$transaction->txn_id} Your new balance is Tk{$fromUser->wallet->balance}. Thanks for using {$app}.", "Send money {$sendAmount} ", $fromUser->phone, null, $fromUser->name);

            // Return success response
            return response()->json([
                'code'    => 'SEND_MONEY',
                'message' => "Send money Tk{$sendAmount} transaction has been completed.",
            ]);
        } catch (\Exception $e) {
            DB::rollBack();
